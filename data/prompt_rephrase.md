...
---
# 指示
対象テキストと意味が変わらないよう、より良いまたはより詳細な表現に書き換えた候補を3つ挙げよ。

各候補は対象テキストを置き換えた結果として**全体で完全な文章**であること。対象テキストが複数の文・複数の行からなる場合は、候補も同じ範囲をすべて含めること（一部だけを返してはならない）。対象テキストが改行を含む場合、候補の中でも改行を `\n` として保持してよい。
